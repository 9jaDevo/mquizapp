/// Dio-based HTTP client for the mQuiz NestJS API.
///
/// Security:
/// - Firebase ID token attached to every request via Authorization header.
/// - On 401: force-refreshes token once and retries automatically.
/// - Auth token never logged (requestHeader: false in LogInterceptor).
/// - Timeouts enforced to prevent hanging requests.
library;
...
import 'package:mquiz/core/network/api_config.dart';
import 'package:mquiz/core/network/api_response.dart';
  ...
  Dio _buildDio() {
    ...
    if (kDebugMode) {
      dio.interceptors.add(LogInterceptor(
        ...
        responseBody: true,
        error: true,
        requestHeader: false, // never log auth tokens
      ));
    }
