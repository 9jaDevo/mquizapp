...
import 'package:mquiz/features/contests/cubit/contest_quiz_cubit.dart';
...
class ContestQuizScreen extends StatelessWidget {
  const ContestQuizScreen({
    super.key,
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) => ContestQuizCubit(
        context.read<ContestRepository>(),
        contestId,
      )..start(),
      child: _ContestQuizView(contestId: contestId, contestName: contestName),
    );
  }
}

class _ContestQuizView extends StatelessWidget {
  const _ContestQuizView({required this.contestId, this.contestName});
  ...
  @override
  Widget build(BuildContext context) {
    return BlocConsumer<ContestQuizCubit, ContestQuizState>(
      listenWhen: (_, cur) =>
          cur is ContestQuizCompleted || cur is ContestQuizError,
      listener: (ctx, state) {
        if (state is ContestQuizCompleted) {
          ctx.pushReplacement(
      ...
      builder: (context, state) => Scaffold(
        ...
        body: SafeArea(
          child: switch (state) {
            ...
            ContestQuizSubmitting() => const _SubmittingView(),
            ContestQuizError(message: final msg) => ErrorStateView(
                message: msg,
                onRetry: () => context.pop(),
              ),
            ContestQuizInProgress() =>
              _QuizPlayView(state: state, cubit: context.read()),
            ContestQuizCompleted() =>
              const Center(child: CircularProgressIndicator()),
          },
        ),
      ),
...
class _QuizPlayView extends StatelessWidget {
  ...
  final ContestQuizInProgress state;
  final ContestQuizCubit cubit;
