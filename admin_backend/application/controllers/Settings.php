...
class Settings extends CI_Controller
{
    ...
    public function referral_dashboard()
    {
        if (!$this->session->userdata('isLoggedIn')) {
            ...
            $this->load->view('referral_dashboard');
        }
    }

    public function referral_fraud_review()
    {
        if (!$this->session->userdata('isLoggedIn')) {
            redirect('/');
        } else {
            $this->load->view('referral_fraud_review');
        }
    }
    ...
    public function referral_activity()
    {
        if (!$this->session->userdata('isLoggedIn')) {
            ...
            $this->load->view('referral_activity');
        }
    }

    public function referral_settings()
    {
        if (!$this->session->userdata('isLoggedIn')) {
            redirect('/');
        } else {
            $this->load->view('referral_settings');
        }
    }
    ...
    public function save_referral_settings()
    {
        ...
        if ($this->input->post()) {
            // Settings to update
            $settings = [
                'referral_bonus_system_enable',
                'referral_reward_min_active_days',
                'referral_reward_min_quizzes',
                'referral_bonus_referrer_coins',
                'referral_bonus_referee_coins',
                'referral_max_per_day',
                'referral_max_per_device',
                'referral_same_ip_max_count',
                'referral_block_same_ip',
                'referral_verify_device_unique',
                'referral_verify_email_unique'
            ];
            ...
            foreach ($settings as $setting) {
                ...
                $exists = $this->db->where('type', $setting)->get('tbl_settings')->row();
                if ($exists) {
                    $this->db->where('type', $setting)->update('tbl_settings', ['message' => $value]);
                } else {
    ...
    public function resolve_fraud()
    {
        ...
        $fraud_id = $this->input->post('fraud_id');
        $referral_id = $this->input->post('referral_id');
        $action = $this->input->post('action'); // 'approve' or 'reject'

        if ($action == 'approve') {
            // Mark fraud check as resolved
            $this->db->where('id', $fraud_id)->update('tbl_referral_fraud_checks', [
                'resolved' => 1,
                'resolved_at' => date('Y-m-d H:i:s')
            ]);

            echo json_encode(['error' => false, 'message' => 'Approved']);
        } else if ($action == 'reject') {
            // Mark fraud check as resolved and reject referral
            $this->db->where('id', $fraud_id)->update('tbl_referral_fraud_checks', [
                'resolved' => 1,
                'resolved_at' => date('Y-m-d H:i:s')
            ]);

            $this->db->where('id', $referral_id)->update('tbl_referrals', [
                'status' => 'rejected',
                'rejection_reason' => 'Manually rejected by admin'
            ]);

            echo json_encode(['error' => false, 'message' => 'Rejected']);
        }
    }
}
